...
const Carousel: FC<CarouselProps> = ({ slice }) => {
  ...
  function changeFlavor(index: number){
    ...
    tl.to(
      ...
      {
        y: 
          index > currentFlavorIndex ? `-=${Math.PI * 2 * SPINS_ON_CHANGE}` : `+= ${Math.PI * 2 * SPINS_ON_CHANGE}`,
          ease: "power2.inOut",
          duration: 1,
      },
      0,
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center">
        {/* left */}
        <ArrowButton onClick={() => changeFlavor(currentFlavorIndex + 1)} direction="left" label="Next Flavor"/>
