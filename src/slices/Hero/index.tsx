...
const Hero: FC<HeroProps> = ({ slice }) => {
  ...
  const ready = useStore((state) => state.ready); // to know if to start the animation
  ...
  useGSAP(() =>{
    ...
    1,
    ).from(".text-side-heading .split-char",{ //split char is each letter
      scale: 1.3,
      y: 40,
      rotate: -25,
      opacity: 0,
  ...
  return (
    ...
    >
      {/* if we are on a smaller screen, we only want to render what is in the view */}
      {isDesktop && (
        <View className="hero-scene pointer-none sticky top-0 z-50 -mt-[100vh] hidden h-screen w-screen md:block">
          <Scene/>
