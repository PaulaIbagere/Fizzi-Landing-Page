...
export default function Scene({sentence, flavor}: skyDiveProps) {
    ...
    const ANGLE = 75 *(Math.PI / 180)// geting 1 degree in radients and multiplying it by 75 to get 75 degrees
  ...
  return (
    <group ref={groupRef}>
        ...
        {/* Text */}
        <group ref={wordsRef}>
            {sentence && <ThreeText sentence={sentence} color="#597215"/>}
        </group>
...
function ThreeText({sentence, color = "white"}: {sentence: string, color: string}){
  const word = sentence.toUpperCase().split(" "); //to split the words
  const material = new THREE.MeshLambertMaterial();
  const isDesktop = useMediaQuery("(min-width: 950px)", true);

  //to return a text element for each word
